...
func GetProductByIdFromDB(productID string) (*models.Product, error) {
	...
	if err := utils.ExecGetQuery(query, []interface{}{productID}, &product); err != nil {
		// If no rows found
...
func AddProductToDB(product *models.Product) (string, error) {
	...
	if err = utils.ExecGetTransactionQuery(
		tx,
		query,
		[]interface{}{product.Name, product.Description, product.Price, product.Stock},
		&productID,
	); err != nil {
		// If no rows found
		if errors.Is(err, sql.ErrNoRows) {
...
func PutUpdateProductInDB(product *models.Product, productID string) error {
	...
	if err := utils.ExecGetTransactionQuery(
		tx,
		query,
		[]interface{}{product.Name, product.Description, product.Price, product.Stock, productID},
		&updatedProductID,
	); err != nil {
